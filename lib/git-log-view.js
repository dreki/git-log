...
GitLogView.prototype.get_log = function() {
    ...
    var settings = atom.config.get();
    var maxCommits = settings['git-log'].maxCommitCount;
    var args = ['log', '--decorate=full', '--date=default', '--pretty=fuller', '--all', '--parents', '--numstat', '--topo-order', '--raw', '--max-count=' + maxCommits, '--first-parent', '--no-merges'];
    var options = {};
...
GitLogView.prototype.fill_content = function() {
    var create_main_row = function(log) {
        var html = '<tr><td class="graph-cell"><p> &nbsp;</p></td>';
